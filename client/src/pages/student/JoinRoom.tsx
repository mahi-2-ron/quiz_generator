...
export default function JoinRoom() {
  ...
  const onJoin = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post(`/rooms/${code.toUpperCase()}/join`);
      navigate(`/room/${code.toUpperCase()}/lobby`);
    } catch (err: any) {
      setError(err.response?.data?.message || 'That code doesn\'t seem right! 🧐');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md space-y-12 relative z-10 animate-slide-in">
        <div className="text-center space-y-4">
          <div className="inline-flex p-4 rounded-3xl bg-white shadow-xl animate-floating-1 mb-4">
             <Zap size={48} className="text-[var(--yellow)] fill-[var(--yellow)]" />
          </div>
          <h2 className="text-5xl font-heading text-[var(--text)] tracking-tight">Ready to Battle?</h2>
        ...
        <div className="card p-10 bg-white/90 backdrop-blur-sm border-none shadow-[0_30px_60px_rgba(155,93,229,0.15)]">
          {error && (
            <div className="mb-6 animate-pop text-sm font-black text-[var(--red)] bg-[var(--red-l)] p-4 rounded-2xl border-2 border-[var(--red)] flex items-center gap-3">
               <span>⚠️</span> {error}
            </div>
          )}
          
          <form className="space-y-8" onSubmit={onJoin}>
            <div className="input-group">
               <label className="label text-center block w-full mb-4">Game Pin</label>
               <input
                 type="text"
                 maxLength={6}
                 value={code}
            ...
            <button 
               ...
               className={cn(
                 ...
                 (loading || code.length < 2) && "opacity-50 grayscale cursor-not-allowed"
               )}
            >
              {loading ? <Loader2 className="w-8 h-8 animate-spin" /> : <><ShieldCheck size={28} /> ENTER ARENA</>}
            </button>
          </form>
        </div>

        <div className="flex justify-center gap-8 opacity-40 grayscale blur-[1px]">
